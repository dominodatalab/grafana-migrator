...
Operator** custom resources for an **operator-managed Grafana instance**,
**skipping anything that's already there** so re-running the tool -- or
running it against a target where some content has already been migrated
by hand -- doesn't create duplicates.
...
- Network reachability from wherever you run this tool to the source
  ...
  prefix (see `--source-path-segment`), or `kubectl port-forward -n
  <namespace> svc/<grafana-service> <local-port>:80` if it isn't otherwise
  reachable.
- **Credentials, preferably a Grafana service account token**
  ...
  search/fetch still works, which reads as a confusing partial failure. A
  token also isn't affected by the admin-password gotcha described in
  Known limitations, and doesn't require knowing the admin username.
  `--source-user`/`--source-password` (basic auth) is supported as a
  fallback with the same Admin-role requirement -- and note **the admin
  username is not necessarily `admin`**; check the deployment's
  `GF_SECURITY_ADMIN_USER` setting or the pod's startup logs.
...
- **A `Grafana` custom resource must already exist** in the target
  ...
  `--instance-selector` (e.g. a Helm-installed instance commonly carries
  `dashboards: <release-name>`, `alerts: <release-name>`, etc.). Every
  generated CR's `spec.instanceSelector.matchLabels` is stamped from this
  flag verbatim -- get the label wrong and every manifest applies cleanly
  but nothing shows up in Grafana, since the operator has nothing to match
  it against.
...
cd grafana-migrator
pip install -e .
grafana-migrator --help
```

`pip install -e .` registers the `grafana-migrator` console script (see
`[project.scripts]` in `pyproject.toml`) and installs `requests`/`pyyaml`.
Without installing, `python3 -m grafana_migrator ...` works identically
from the repo root as long as `requests`/`pyyaml` are already on your
`PYTHONPATH`.
...
Capture a snapshot of the source instance -- no target cluster required
yet:

```bash
export GRAFANA_SOURCE_TOKEN='glsa_...'   # service account token, preferred

grafana-migrator export \
  --source-url https://grafana.example.com \
  --output-dir ./grafana-migrator-source
```

This prints a short summary (dashboard/folder/alert-rule/contact-point
counts, whether a notification policy was found) and writes the raw
snapshot to `./grafana-migrator-source/`. Pass `--dry-run` to only print
the summary and skip writing anything, e.g. to sanity-check reachability
and credentials before capturing the real thing.

Once a target operator-managed instance exists, dedup the snapshot against
it and see what's genuinely new -- always start with `--dry-run`:

```bash
grafana-migrator import ./grafana-migrator-source \
  --namespace monitoring \
  --instance-selector dashboards=my-grafana \
  --output-dir ./grafana-migrator-manifests \
  --dry-run
```

This prints the full report but writes nothing -- a safe way to sanity-check
counts (how many dashboards/rules/contact points are genuinely new, how many
title-collisions need a look) before anything touches disk or the cluster.
Read the "Title-collision skips" section of the report if it's non-empty --
those are dashboards this tool is deliberately *not* migrating by default.

Once the dry-run report looks right, drop `--dry-run` to actually write
manifests:
...
`report.json`), plus `report.json` describing every decision made, and
prints the same text report to stdout. Every manifest is plain, readable
YAML -- multi-line content like a dashboard's embedded JSON is written as
a proper indented block, not an escaped one-line string, so the files are
meant to be opened and reviewed before anything is applied.
...
Or pass `--apply` to `import` to write and apply in one step (mutually
...
Or export with basic auth instead of a token:
...
# once it exists:
grafana-migrator import ./grafana-migrator-source \
  --namespace monitoring --instance-selector dashboards=my-grafana \
...
grafana-migrator import ./snapshot --namespace monitoring \
  --instance-selector dashboards=my-grafana \
...
migrated/skipped counts look surprising -- every limitation below that
affects dedup behavior shows up here first.

## Known limitations

### Not yet implemented

- **Data sources are not yet handled.** Still open work.
